...
pub enum Mining<'a> {
    ...
    SubmitSharesExtendedEHash(SubmitSharesExtendedEHash<'a>),
...
impl fmt::Display for Mining<'_> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Mining::SubmitSharesExtendedEHash(m) => write!(f, "{m}"),
...
impl Mining<'_> {
    ...
    pub fn into_static(self) -> Mining<'static> {
        match self {
            ...
            Mining::SubmitSharesExtendedEHash(m) => Mining::SubmitSharesExtendedEHash(m.into_static()),
...
impl IsSv2Message for Mining<'_> {
    fn message_type(&self) -> u8 {
        match self {
            ...
            Self::SubmitSharesExtendedEHash(_) => MESSAGE_TYPE_SUBMIT_SHARES_EXTENDED_EHASH,
    ...
    fn channel_bit(&self) -> bool {
        match self {
            ...
            Self::SubmitSharesExtendedEHash(_) => CHANNEL_BIT_SUBMIT_SHARES_EXTENDED,
...
impl<'decoder> From<Mining<'decoder>> for EncodableField<'decoder> {
    fn from(m: Mining<'decoder>) -> Self {
        match m {
            ...
            Mining::SubmitSharesExtendedEHash(a) => a.into(),
...
impl GetSize for Mining<'_> {
    fn get_size(&self) -> usize {
        match self {
            ...
            Mining::SubmitSharesExtendedEHash(a) => a.get_size(),
