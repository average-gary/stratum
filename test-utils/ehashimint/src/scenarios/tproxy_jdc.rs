...
use crate::config::{self, defaults, PoolConfig, TProxyConfig, JdcConfig, JdsConfig};
...
///
/// In this setup:
/// - JDC mints eHash tokens locally
/// - Pool validates shares (no minting)
...
pub async fn run(
    ...
    info!("═══════════════════════════════════════════════════════");
    info!("TProxy with JDC Mint and JDS - eHash Testing Environment");
    info!("═══════════════════════════════════════════════════════");
    info!("");
    info!("Configuration:");
    info!("  • Pool: Validates shares (no eHash mint)");
    info!("  • JDS: Provides job declaration service");
    info!("  • JDC: Mints eHash tokens locally");
    info!("  • TProxy: SV1→SV2 translation + correlation tracking");
    info!("  • Protocol: SV1 miners → TProxy → JDC → Pool");
    info!("");
    ...
    let pool_config = if let Some(path) = pool_config_path {
        ...
        defaults::pool_config(false) // mint_enabled = false
    };
    ...
    info!("  • TProxy: {}", tproxy_config_file.display());
    info!("");
