...
pub async fn spawn_mint_thread(
    ...
    let mut mint_handler = ehash_integration::mint::MintHandler::new(config.clone()).await.map_err(|e| {
    ...
    let router = cdk_axum::create_mint_router(mint, false)
        .await
        .map_err(|e| {
            JDCError::Custom(format!("Failed to create HTTP router: {}", e))
        })?;
